...
// The 45-degree planar bevel face of a chamfered vertical corner: normal is
// horizontal (Z~0) but diagonal (both X and Y components significant).
TopoDS_Face blendCyl(const TopoDS_Shape& body) {
    for (TopExp_Explorer ex(body, TopAbs_FACE); ex.More(); ex.Next()) {
        ...
        if (std::abs(n.Z()) > 0.05) continue;
        if (std::abs(n.X()) > 0.2 && std::abs(n.Y()) > 0.2) return f; // diagonal
    }
    return {};
}
...
TEST(TopoChamferGen, BevelFaceNameFollowsResize) {
    Document doc;
    int pid[4];
    ...
    EXPECT_EQ(ref.names.front().scheme, "gen")
        << "a bevel face is nameable only by generation lineage";
    EXPECT_LT(centroidX(blend1), 20.0) << "bevel sits at the x=20 corner";
    ...
    ASSERT_TRUE(ext.rebuildProfileFromSketch(doc));
    ASSERT_TRUE(ext.execute(doc));
    ASSERT_TRUE(fil.execute(doc)) << "chamfer follows the moved corner";
    ...
    ASSERT_TRUE(topo::resolve(ref, ctx2, out))
        << "the bevel-face lineage name must resolve on the resized body";
    ASSERT_EQ(out.ShapeType(), TopAbs_FACE);
    EXPECT_GT(centroidX(TopoDS::Face(out)), 30.0)
        << "resolved bevel must be at the MOVED corner (x~40), not the old x=20";
